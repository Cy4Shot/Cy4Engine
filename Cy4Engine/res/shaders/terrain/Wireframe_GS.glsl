...
void main() {
	...
	if (dist < tbn_range) {
		...
		for (int k = 0; k < gl_in.length(); k++) {
			...
			float scale = 0;
			for (int i = 0; i < 3; i++) {
				scale += texture(materials[i].heightmap,
						mapCoord_GS[k] * materials[i].horizontalScaling).r
						* materials[i].heightScaling * blendValueArray[i];
			}

			float attenuation = clamp(
					-distance(gl_in[k].gl_Position.xyz, cameraPosition)
							/ (tbn_range - 50) + 1, 0.0, 1.0);
			scale *= attenuation;
