...
//		setUniformf("scaleY", terrainNode.getConfig().getScaleY());
		...
		for (int i = 0; i < 3; i++) {
			...
			setUniformf("materials[" + i + "].heightScaling",
					terrainNode.getConfig().getMaterials().get(i).getDisplaceScale());
			setUniformf("materials[" + i + "].horizontalScaling",
					terrainNode.getConfig().getMaterials().get(i).getHorizontalScale());
		}
	}
}
