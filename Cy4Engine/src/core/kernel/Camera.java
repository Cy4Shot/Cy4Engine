...
public class Camera {
	...
	protected Camera() {
		...
		setViewMatrix(new Matrix4f().View(this.getForward(), this.getUp())
				.mul(new Matrix4f().Translation(this.getPosition().mul(-1))));
	...
	public void update() {
		...
		if (Input.getInstance().isButtonHolding(2)) {
			float dy = Input.getInstance().getLockedCursorPosition().getY()
					- Input.getInstance().getCursorPosition().getY();
			float dx = Input.getInstance().getLockedCursorPosition().getX()
					- Input.getInstance().getCursorPosition().getX();
			...
			glfwSetCursorPos(Window.getInstance().getWindow(), Input.getInstance().getLockedCursorPosition().getX(),
	...
	public void setProjection(float fovY, float width, float height) {
		...
		this.projectionMatrix = new Matrix4f().PerspectiveProjection(fovY, width, height, Constants.ZNEAR,
				Constants.ZFAR);
	}
